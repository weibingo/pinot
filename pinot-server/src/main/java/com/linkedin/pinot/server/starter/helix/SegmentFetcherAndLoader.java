  ...
  public void addOrReplaceOfflineSegment(String tableName, String segmentId, boolean retryOnFailure) {
    ...
    try {
      ...
      if (localSegmentMetadata == null) {
        ...
        // Reload failure might leave segment in inconsistent state (index directory might not exist but segment backup
        // segment existed), need to first try to recover from reload failure before checking the existence of the index
        // directory and loading metadata from it
